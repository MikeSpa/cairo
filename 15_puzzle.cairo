...
func verify_valid_location(loc: Location*) {
    //loc is address of a Location, so loc is the row and loc+1 is the col value

    // Check that row is in the range 0-3.
    // temporary variable, memory in cairo is immutable, tempvar scope is restricted (can be revoked in if or fct call)
    tempvar row = loc.row;
    // no < in cairo, need to use builtin or lib fct like like assert_nn_le()
...
func verify_adjacent_locations(
    ...
    // can access local variable inside the entire function
    alloc_locals;
...
func main{output_ptr: felt*, range_check_ptr}() {
    ...
    local solution: (felt, felt, felt, felt) = (3, 7, 8, 12);
